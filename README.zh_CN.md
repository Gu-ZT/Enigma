...
ETP/1 仍处于实验阶段。仓库现在包含固定目标和无认证 SOCKS5 客户端/服务端命令，但
尚未提供 HTTP CONNECT、连接复用或 UDP。
...
5. **代理协议有限：** 当前命令支持固定目标和无认证 SOCKS5，但 HTTP CONNECT、TUN、
   mux、fallback、HTTP 伪装和 UDP 仍未实现。
...
cmd/enigma/       keygen、服务端和固定目标客户端命令
```
